...
def answer_question(
...
) -> Tuple[str, List[str], str, bool, Optional[float]]:
    """질문에 답변. (answer, contexts, conversation_id, cache_hit, cache_similarity)"""
    ...
    if cache_enabled:
        try:
            semantic_cache.store(question, answer, contexts)
        except Exception:
            logger.exception("semantic cache store failed; continuing")

    return answer, contexts, conversation_id, False, None
